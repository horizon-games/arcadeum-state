...
fn test_coin() {
    ...
    // This is returned as ProofAction structs, so you can use debug or serialized repesentations to make assertions.
    let revealed_data = format!("{:#?}", tester.apply(Some(0), &true).unwrap());
